...
- **CRITICAL - NO PDFs IN PLAN:** Do NOT instruct the `ResearchAgent` to "read", "view", or "start with" any `.pdf` files. The `ResearchAgent` is completely incapable of interacting with PDF files. If a `semantic_search` surfaces relevant information from a `.pdf` file, the content is ALREADY extracted in the search result. Instruct the `ResearchAgent` to rely directly on the search results or to run its own `semantic_search` queries for further information, but NEVER tell it to open a PDF.
